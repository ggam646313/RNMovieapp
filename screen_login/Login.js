...
export default class CheckLogin extends Component{
        ...
        return(
            <View style={style.root}>
                ...
                <View style={style.content}>
                    ...
                    {/* 1.2 이메일 / 패스워드 입력박스 */}
                    {/* TextInput은 로그인, 회원가입, 비밀번호 리셋화면에 모두 사용되므로 사용빈도가 높음 */}
                    ...
                    {/* 1.4 로그인 버튼 */}
                    {/* 가로 전체 사이즈의 버튼을 만들기 위해 View로 감쌌다 */}
                ...
                <View style={style.footer}>
                    <Text style={style.footerCopyright}>MOVIEAPP by ggam</Text>
                </View>
            </View>
        );
    } // render
    ...
    login= async ()=>{
        // 원래는 서버에 전송하는 코드사용해야 하지만 시간상 생략 !
        
        // 대신 다음에 또 로그인하면 사용자가 귀찮아 하므로
        // 디바이스에 로그인 유무를 저장
        await AsyncStorage.setItem('isLogin','email'); // 임시로 'email'이라는 글씨 저장

        // 로그인 되었다면 앱의 주요화면인 MovieList.js 를 가진 DrawerNav로 이동
        this.props.navigation.navigate('drawerNav');
    }

    // 네비게이터 옵션 [static은 새로고침으로 반영되지 않음. 반드시 re-run 해야함]
